...
if isfolder(filefolder)
    ...
    excels = excels(3:end);
    names = {excels(:).name};
    fileIOR = readtable(fullfile(filefolder, names{1,4}));
    disp("okay");
    start = 130;
    ...
    labels = {'Resting','Walking', 'Resting', 'Running',...
              'Resting','Walking', 'Running','Ending'};

    time_points = [time_OEP(1), 119,249, 324,459,535,582,601,time_OEP(end)];
...
for count =1:7
    subplot(7,1,count)
    plot(activities(count).Time,activities(count).Pressure,'b')
    ...
    filteredData = applyButterworthFilter(activities(count).Pressure,54,0);
    hold on
    plot(activities(count).Time,filteredData,'r')
    [cbpm,~] = countBreathsPerMinute(activities(count).Time,filteredData);
    text(activities(count).Time(1),min(activities(count).Pressure),num2str(cbpm),"FontWeight",'bold','HorizontalAlignment','left')
    title(activities(count).Label)
end
%%
figure() 
for count =1:7
    subplot(1,7,count)
    histogram(activities(count).Pressure,'Normalization','pdf','DisplayStyle','stairs')
    title(activities(count).Label)
    xline(0,'r')
    if count==1
        ylabel('Pdf')
        xlabel('Pressure Values')
    end
end
...
for count = 1:7
    ...
    xData = activities(count).Pressure;
    [aX, varX] = aryule(xData, order);
    [hX, wX]   = freqz(sqrt(varX), aX, NFFT, fs);
    psdX       = 20*log10(abs(hX));
    
    plot(wX, psdX, 'b', 'DisplayName','AR PSD Z')
    hold on 
...
function filteredData = applyButterworthFilter(data, fs,plotG)
    ...
    %
    % Returns:
    % filteredData - Filtered signal

    % Define filter specifications
    fLow = 0.025; % Low cutoff frequency (Hz)
    fHigh = 1;    % High cutoff frequency (Hz)
    ...
    if (plotG)
        ...
        ylabel('Magnitude');
        grid on;
        xlim([0, 2]); % Adjust based on filter range
    end 
end
