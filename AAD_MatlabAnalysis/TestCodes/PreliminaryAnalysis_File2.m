...
if isfolder(filefolder)
    ...
    excels = excels(3:end);
    names = {excels(:).name};
    fileIOR = readtable(fullfile(filefolder, names{1,2}));
    disp("okay");
    start = 1;
    ...
    figure()
    plot(time_OEP,accz,'-x');
    hold on 
    plot(time_OEP,accy,'-x');
    plot(time_OEP,accx,'-x');
    labels = {'Resting', 'Running', 'Resting', 'Walking'};
    time_points = [time_OEP(1), 157.221, 278, 398, 491];
